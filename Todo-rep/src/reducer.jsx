...
const deleteTodoListAC = (todoId) => ({type: DELETE_TODOLIST, todoId});
export const deleteTaskAC = (taskId, todoId) => ({type: DELETE_TASK, taskId, todoId});
const setTaskAC = (todoListId, tasks) => ({type: SET_TASKS, todoListId, tasks});
const changeTodoTitleAC = (todoId, title) => ({type: SHANGE_TODOTITLE, todoId, title});
...
export const setTaskTC = (todoListId) => (dispatch) => {api._restoreState2(todoListId)
        .then(res => {let tasks = res.data.items; dispatch(setTaskAC(todoListId, tasks))  }) };

export const deleteTaskTC = (taskId, todoId) => (dispatch) => {api.deleteTask(taskId,todoId)
      .then(res => {if (res.data.resultCode === 0) dispatch(deleteTaskAC(taskId, todoId)) }) };

export const deleteTodoListTC = (todolistId) => (dispatch) => {api.deleteTodolist(todolistId)
    .then(res => {if (res.data.resultCode === 0) dispatch(deleteTodoListAC(todolistId)) }) };

export const addTaskTC = (newText,todoId) => (dispatch) => {api.createTask(newText,todoId)
    .then(res => { let newTask = res.data.data.item; dispatch(addTaskAC(todoId, newTask)) })
};

export const changeTodoTitleTC = (todoId, title) => (dispatch) => {api.changeTodoTitle(todoId, title)
    .then(res => { dispatch(changeTodoTitleAC(todoId, title)) })
};


export const changeTaskTC = (taskId, updateTask, todoId) => (dispatch) => {api.changeTask(taskId, updateTask, todoId)
